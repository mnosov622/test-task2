...
route.addEventListener("change", () => {
  ...
  if (route.value === "из A в B и обратно в А") {
    ...
    travelHours = `${Math.floor(travelTime / 60)} ${numWord(travelHours, [
      "часов",
      "часа",
      "час",
    ])}`;
...
departureTime.addEventListener("change", (e) => {
  ...
  let timeAsString = departureTime.value.substring(
    0,
    departureTime.value.indexOf("(").toString()
  );
  ...
  if (sortedTimes.length === 0) {
    ...
    timeOptionNotFound.innerHTML =
      "К сожалению, на выбранное время нет обратного теплохода";
...
ticketsForm.addEventListener("submit", (e) => {
  ...
  let travelBackTimeAsString = travelBackTime.value.substring(
    0,
    travelBackTime.value.indexOf("(").toString()
  );
  ...
  const travelBackArrTime =
    (travelBackH < 10 ? "0" : "") +
    travelBackH.toString() +
    ":" +
    (travelBackMin < 10 ? "0" : "") +
    travelBackMin.toString();
  ...
  let travelBackTimeTotal =
    (arrBackHour < 10 ? "0" : "") +
    arrBackHour.toString() +
    ":" +
    (arrBackMin < 10 ? "0" : "") +
    arrBackMin.toString();

  summary.innerHTML += `Вы выбрали <b>${ticketsAmount.value} ${numWord(
    ticketsAmount.value,
    ["билет", "билета", "билетов"]
  )}</b> по маршруту <b>${route.value}</b> стоимостью <b>${ticketPrice}р</b>.
    <br>
    ...
    ${
      travelBack
        ? `Теплоход из <b>A в B</b>  отправляется в <b>${depTime}</b>, а прибудет в <b>${shipArrivalTimeTotal}.</b>`
    ...
    ${
      travelBack
        ? `  Теплоход из <b>B в A</b> отправляется в <b>${travelBackArrTime}</b>, а прибудет в <b>${travelBackTimeTotal}</b> `
        : ""
    }
    `;
});
