...
cards.forEach((card) => {
  ...
  cardTimesAvailable.forEach((cardTimeAvailable) => {
    const timeCards = cardTimeAvailable.querySelectorAll(
      ".card__time-available"
    );
    const expandMoreBtn = cardTimeAvailable.querySelector(
      ".card__moreTimes-btn"
    );
    const hiddenTimes = cardTimeAvailable.querySelectorAll(
      ".card__times-hidden"
    );
